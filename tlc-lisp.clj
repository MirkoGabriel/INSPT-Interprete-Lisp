...
(defn controlar-aridad [lis val-esperado] (
	cond
    (igual? (count lis) val-esperado) val-esperado
...
(defn imprimir
   ([elem] (
	   cond
       (igual? elem \space) (do (flush) '\space)
       (seq? elem) (
                        if
...
(defn actualizar-amb [amb-global clave valor] 
  ( 
   if
   (and (seq? valor) (igual? '*error* (first valor)))
   amb-global
   (if
    ...
    ((fn ciclo [a b] (
        if
        (igual? (first a) clave)
        (reverse (into (list) (assoc (into [] amb-global) (inc b) valor)))
